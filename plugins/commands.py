...
async def approve_new(client, m: ChatJoinRequest):
    ...
    try:
        ...
        # Required tags to look for (case-insensitive)
        required_tags = ["@real_pirates", "@drama_loverx", "@ii_way_to_success_ii", "@myownsuccess", "@goal_achieverr", "@just_vibing_alone"]
...
Your Request To Join <b>{m.chat.title}</b> Has Been Approved!✅
...
<blockquote>Supported by <b>💀 @Real_Pirates 🏴‍☠️</blockquote></b>"""
            ...
            # Log the approval
            try:
                await client.send_message(-1002326947174, approve_text)  # Replace with your log channel ID
            except Exception as e:
...
For Study-Skills Channel 
🔰🔰🔰🔰🔰🔰
• <code>@II_Way_to_Success_II</code>
• <code>@Goal_Achieverr</code>  
• <code>@Myownsuccess</code> 
• <code>@Just_Vibing_Alone</code>  
<em>Tap to Copy 👆</em>

Once You've Added Any Of This In Your Bio,
return and try joining again using the invite link — I'll be happy to approve your request!</b> 🥰"""
                )
